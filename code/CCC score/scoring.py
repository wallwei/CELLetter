...
def load_data():
    ...
    if 'non-cancer cell type' in meta_filt.columns:
        ...
        print(" 警告: 元数据中没有 'non-cancer cell type' 列")
    ...
    if 'classified as cancer cell' in meta_filt.columns:
        ...
        print(" 警告: 元数据中没有 'classified as cancer cell' 列")
...
def calculate_cell_communication(auc_mtx, expr_filt, meta_filt, lr_pairs):
    ...
    # 5. 计算TF活性代表值
    print("计算各细胞类型的TF活性代表值...")
    cell_type_tf_activity = {}  # 创建一个字典来存储每个细胞类型的活性值

    for cell_type in unique_cell_types:
        # 获取属于当前细胞类型的所有细胞ID
        cell_ids_of_this_type = cell_types[cell_types == cell_type].index
        # 找出这些细胞ID在auc_mtx中也存在的（求交集）
        valid_cell_ids = [cid for cid in cell_ids_of_this_type if cid in auc_mtx.index]

        if len(valid_cell_ids) == 0:
            # 如果没有有效细胞，则活性设为0
            cell_activity_score = 0
        else:
            # 提取这些细胞的TF活性数据
            cell_activity_data = auc_mtx.loc[valid_cell_ids].astype(float)
            # 计算这些细胞TF活性的平均值（先取log1p，再按列求平均，最后对所有TF求平均）
            try:
                tf_mean_activities = np.log1p(cell_activity_data.values).mean(axis=0)
                cell_activity_score = tf_mean_activities.mean()
            except Exception as e:
                print(f"计算细胞类型 {cell_type} 的TF活性时出错: {e}")
                cell_activity_score = 0

        # 将结果存入字典
        cell_type_tf_activity[cell_type] = cell_activity_score
        print(f"   {cell_type}: {cell_activity_score:.4f}")
    ...
    for chunk_start in range(0, total_pairs, chunk_size):
        ...
        for _, pair in chunk.iterrows():
            ...
            for sender_type in unique_cell_types:
                # 获取发送方细胞类型的TF活性，而不是全局活性
                sender_tf_activity = cell_type_tf_activity[sender_type]
                # 如果发送方活性不满足阈值，跳过该发送方
                if sender_tf_activity < MIN_TF_ACTIVITY:
                    continue
                ...
                for receiver_type in unique_cell_types:
                    ...
                    weighted_score = base_score * sender_tf_activity  # 修改这里
    ...
    print(f"存严格过滤后的LRI对至: filtered_lri_pairs.csv")
...
def main():
    ...
    print(f"终过滤后的LRI对数量: {len(final_filtered_lri)}")
